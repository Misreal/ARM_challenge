"""Score one candidate's optval accuracy on the device, in its own process.
...
def score(spec: BenchSpec, limit: int | None, bundle_dir: Path) -> dict[str, Any]:
    """Top-1/top-5 over the first `limit` optval images, or all of them."""
...
def main() -> None:
    ...
    result = score(spec, args.limit, args.bundle_dir)
